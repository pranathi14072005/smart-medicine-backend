...
public class MedicineService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MedicineService.class);
    ...
    public MedicineDTO getMedicineById(Long id, Long userId) {
        Medicine medicine = medicineRepository.findByIdAndUserId(id, userId).orElseThrow(() -> new ResourceNotFoundException("Medicine", "id", id));
        return toDTO(medicine);
    }
    ...
    public List<MedicineDTO> searchMedicines(String name, Long userId) {
        return medicineRepository.searchByNameAndUserId(name, userId).stream().map(this::toDTO).collect(Collectors.toList());
    }

    @Transactional
    public MedicineDTO updateMedicine(Long id, MedicineDTO dto, User currentUser) {
        Medicine medicine = medicineRepository.findByIdAndUserId(id, currentUser.getId()).orElseThrow(() -> new ResourceNotFoundException("Medicine", "id", id));
    ...
    public List<MedicineDTO> getExpiredMedicines(Long userId) {
        return medicineRepository.findExpiredMedicines(userId, LocalDate.now()).stream().map(this::toDTO).collect(Collectors.toList());
    }

    public List<MedicineDTO> getExpiringSoon(Long userId, int daysAhead) {
        return medicineRepository.findExpiringSoon(userId, LocalDate.now(), LocalDate.now().plusDays(daysAhead)).stream().map(this::toDTO).collect(Collectors.toList());
    }

    public List<MedicineDTO> getLowStockMedicines(Long userId) {
        return medicineRepository.findLowStockMedicines(userId).stream().map(this::toDTO).collect(Collectors.toList());
    }
    ...
    public Map<String, Object> getMedicineStats(Long userId) {
        ...
        return Map.of("total", total, "active", active, "expired", expired, "lowStock", lowStock, "expiringSoon", medicineRepository.findExpiringSoon(userId, LocalDate.now(), LocalDate.now().plusDays(30)).size());
    }
    ...
    @java.lang.SuppressWarnings("all")
    public MedicineService(final MedicineRepository medicineRepository, final ModelMapper modelMapper) {
